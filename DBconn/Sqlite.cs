...
namespace DBconn
{
    public class Sqlite : ISDb, IDisposable
    {
        ...
        /// <returns></returns>
        public IDataParameter MyParams(string name, object value) => new SQLiteParameter(name, value);
        ...
        public void Open()
        {
            _connSql = new SQLiteConnection(_dataSql);
        ...
        /// </summary>
        /// <param name="sql">sql语句</param>
        /// <param name="ctype">sql或存储参数</param>
        ...
        private static void PrepareCommand(SQLiteCommand cmd, SQLiteConnection conn, SQLiteTransaction trans, CommandType cmdType, string cmdText, params IDataParameter[] cmdParms)
        {
            cmd.Connection = conn;
        ...
        {
            var cachedParms = (SQLiteParameter[])_parmCache[cacheKey];
            if (cachedParms == null)
                return null;
            var clonedParms = new SQLiteParameter[cachedParms.Length];
            for (int i = 0, j = cachedParms.Length; i < j; i++)
                clonedParms[i] = (SQLiteParameter)cachedParms[i].Clone();
            return clonedParms;
        }
